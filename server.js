...
const mongoDB="mongodb+srv://mir:mir12345@cluster0.xpmh5.mongodb.net/gmitfinder"
...
app.post('/profiledata',myBodyParser,(req,res)=>{
    ...
    dbsql.query("SELECT id ,name,email,phone FROM login_cred WHERE id=?",
    [id],
    (err,result)=>{
        if(err){
            res.send({err:err})
...
app.post('/allcvdata',myBodyParser,(req,res)=>{
    const id = req.body.id
    dbsql.query("SELECT * FROM login_cred WHERE id=?",
    [id],
    (err,result)=>{
        if(err){
            res.send({err:err})
    ...
    dbsql.query(
        ...
        (err,result)=>{
            ...
            if (result.length>0) {
                ...
                var decryPassword = deCipher.update(result[0].password,'hex','utf8')
                +deCipher.final('utf8');
            ...
            if(err){
                ...
                if(result.length!=0){
                    ...
                    if(password===confirm_password){
                        dbsql.query('INSERT INTO login_cred (name, user_type, email, phone, password, gender, date_of_birth) VALUES (?,?,?,?,?,?,?)',
...
app.post("/setflagskill",myBodyParser,(req,res)=>{
    ...
    SkillModel.updateOne({_id:id},{$set:{flag:flag}}).then((details)=>{
        res.status(200).json(details)
    })
})
...
app.post("/setflagacademic",myBodyParser,(req,res)=>{
    const id=req.body.id
    const flag=req.body.flag
    AcademicModel.updateOne({_id:id},{$set:{flag:flag}}).then((details)=>{
        res.status(200).json(details)
    })
})
...
app.post("/setflagachievement",myBodyParser,(req,res)=>{
    const id=req.body.id
    const flag=req.body.flag
    AchievementModel.updateOne({_id:id},{$set:{flag:flag}}).then((details)=>{
        res.status(200).json(details)
    })
})
...
app.post("/setflagproject",myBodyParser,(req,res)=>{
        const id=req.body.id
        const flag=req.body.flag
        ProjectModel.updateOne({_id:id},{$set:{flag:flag}}).then((details)=>{
        res.status(200).json(details)
    })
})
...
app.post("/insertyourinfo",myBodyParser,(req,res)=>{
    ...
    dbsql.query(
        ...
        [name,phone,email,gender,dob,github,linkedin,father_name,address,id],
        (err,result)=>{
            if (err) {
